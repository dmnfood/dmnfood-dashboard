...
export function subscribeToInspectionRecords(onRecords, onError) {
  return onSnapshot(
    query(collection(db, AUX_MATERIAL_INSPECTION_COLLECTION), orderBy('createdAt', 'desc'), limit(100)),
...
export async function saveInspectionRecord(values, user, existingRecord = null) {
  ...
  if (existingRecord) {
    await setDoc(doc(db, AUX_MATERIAL_INSPECTION_COLLECTION, existingRecord.id), payload, { merge: true });
    return existingRecord.id;
  }
  return (await addDoc(collection(db, AUX_MATERIAL_INSPECTION_COLLECTION), { ...payload, createdAt: serverTimestamp() })).id;
}

export const deleteInspectionRecord = id => deleteDoc(doc(db, AUX_MATERIAL_INSPECTION_COLLECTION, id));
