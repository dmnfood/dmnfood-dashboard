...
export function buildHeatingJournalSummary(records, sheetCount) {
  ...
  return [
    ['총 기록', `${sorted.length}건`], ['출력 페이지', `${sheetCount}장`],
    ['1호기', `${count('1')}건`], ['2호기', `${count('2')}건`],
    ['3호기', `${count('3')}건`], ['4호기', `${count('4')}건`],
    ['PASS', `${sorted.filter(record => record.judgement === 'PASS').length}건`], ['FAIL', `${sorted.filter(record => record.judgement === 'FAIL').length}건`],
  ];
}
...
const formatTemperature = value => {
  ...
  const normalized = String(value).replace(/(?:℃|°C)/gi, '').trim();
