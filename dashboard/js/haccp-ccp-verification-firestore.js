...
export const subscribeCcpVerification=(ok,fail)=>onSnapshot(query(collection(db,CCP_VERIFICATION_COLLECTION),orderBy('recordDate','desc'),limit(100)),s=>ok(s.docs.map(d=>({id:d.id,...d.data()}))),fail);
export async function saveCcpVerification(values,user,existing=null){const periodKey=monthKey(values.recordDate);const payload={schemaVersion:1,...values,periodKey,createdByUid:existing?.createdByUid||user.uid,createdByEmail:existing?.createdByEmail||user.email||'',updatedByUid:user.uid,updatedAt:serverTimestamp()};if(!existing)payload.createdAt=serverTimestamp();await setDoc(doc(db,CCP_VERIFICATION_COLLECTION,periodKey),payload,{merge:Boolean(existing)});return periodKey;}
